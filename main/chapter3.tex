...
\begin{figure}[htp]
  \centering
  \subfloat[][Float 32-bit]{\includegraphics[width=0.485\textwidth]{vectorization/mandelbrot_speedup/mandelbrot_speedup_32bit}\label{plot:opt_mipp_mandelbrot_32}}
  \quad
  \subfloat[][Float 64-bit]{\includegraphics[width=0.485\textwidth]{vectorization/mandelbrot_speedup/mandelbrot_speedup_64bit}\label{plot:opt_mipp_mandelbrot_64}}
...
The vectorization is a key to develop high performance implementations of signal
...
\begin{figure}[htp]
  \centering
  \includegraphics[scale=0.9]{vectorization/reordering/reordering}
  \caption
...
Both the scheduling and update rules implementations have a generic real
...
The polar decoder algorithms presented in Section~\ref{sec:alg_polar} has a
...
\begin{figure}[htp]
  \centering
  \includegraphics[scale=1.0]{polar/patterns/patterns}
...
\begin{figure}[htp]
  \centering
  \subfloat[][32-bit intra-frame SIMD (AVX)]{\includegraphics[width=0.485\textwidth]{polar/sc_tree_cut/sc_tree_cut_intra}\label{plot:opt_polar_sc_tree_cut_intra}}
  \quad
  \subfloat[][8-bit inter-frame SIMD (SSE4.1)]{\includegraphics[width=0.485\textwidth]{polar/sc_tree_cut/sc_tree_cut_inter}\label{plot:opt_polar_sc_tree_cut_inter}}
  \caption
...
To go even further a reach highest throughputs and lowest latencies possible,
...
The exact memory footprint of the SSCl decoders is hard to obtain as there are
...
the complex norm $||.||$ in \eqref{eq:alg_scma_5} and \eqref{eq:alg_scma_6},
2) calculate the exponentials in \eqref{eq:alg_scma_7}, 3) perform users to
...
\begin{figure}[htp]
  \centering
  \includegraphics[scale=0.9]{scma/simd_norm/simd_norm_aos}
  \caption
...
\begin{figure}[htp]
  \centering
  \includegraphics[scale=0.9]{scma/simd_norm/simd_norm_soa}
  \caption
...
ways to perform this computation: Fig.~\ref{fig:opt_scma_simd_norm_aos} depicts
how to implement the norm function using an Array of Structures (AoS) for
complex numbers. In this method, the complex numbers are represented as two
consecutive floating-point numbers. The implementation with AoS uses six
intrinsic functions: one load (\verb|_mm256_loadu_ps|), one store
...
AVX register (\verb|_mm256_extractf128_ps|).
Fig.~\ref{fig:opt_scma_simd_norm_soa} sketches the computation of the complex
norm using a Structure of Array (SoA) data layout. This implementation also uses
six intrinsic functions: two loads (\verb|_mm256_loadu_ps|), one store
(\verb|_mm256_storeu_ps|), two standard multiplications (\verb|_mm256_mul_ps|),
one addition (\verb|_mm256_add_ps|).
...
\begin{figure}[htp]
  \centering
  \includegraphics[scale=0.9]{scma/simd_exp/simd_exp}
...
\eqref{eq:alg_scma_7}, the MIPP wrapper~\cite{Cassagne2018} has been used. The
flattened complex and normalized numbers are calculated as shown in
Fig.~\ref{fig:opt_scma_simd_norm_aos} and Fig.~\ref{fig:opt_scma_simd_norm_soa}
to produce the preliminary values used to compute the probabilities.
...
exponential function is performed according to \eqref{eq:alg_scma_7}.

\paragraph{Exponential Approximation with the Estimated-MPA (E-MPA) algorithm}

\eqref{eq:alg_scma_19} can be used as a systematic replacement to the vectorized
exponential MIPP function used in Fig.~\ref{fig:opt_scma_simd_exp}. It reduces
...
\begin{figure}[htp]
  \centering
  \includegraphics[scale=0.9]{scma/simd_final_guess/simd_final_guess}
...
vectorized using SSE instructions. Fig.~\ref{fig:opt_scma_simd_final_guess}
shows the computation of final guesses for user 4. There are four messages from
a resource to a user containing the probabilities of four different codewords,
which are the elements of the SSE vectors. According to
Fig.~\ref{fig:opt_scma_simd_final_guess} these vectors of probabilities are
loaded into SSE, NEON or the lowest lane of the AVX registers.
