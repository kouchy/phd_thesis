...
implementations of the LDPC, polar and turbo decoders. The throughput, the
latency and the energy efficiency are studied and compared with other works.

The forth section summarizes the best software decoder implementations we
found in the literature. Three hall of fames are proposed: one for the LDPC
...
The fifth section is the evaluation of the proposed SCMA demodulator
implementations. The throughput, the latency and the energy efficiency are
explored over various platforms.
...
on two high-end x86 CPUs and on two $\mathcal{H}$ parity matrices that have
different characteristics. Then, the proposed BP decoder is compared with the
...
For the LDPC experimentations, we selected two high end CPUs: the
Intel\R Xeon\TM Platinum 8168 and the AMD\R EPYC 7452 (see
...
on the specialized BP-HL+NMS implementation presented in
...
decoding speed (gains rang between 20\% to 50\% depending on the $\mathcal{H}$
parity matrix and on the CPU used). In all the presented results, the decoder
works on 16-bit fixed-point data. This representation is able to match the
BER/FER decoding performance of the floating-point representation. We encounter
some difficulties to keep an acceptable level of decoding performance when we
ran the 8-bit fixed-point decoder and this is why we choose to only use the
...
\begin{figure}[htp]
  ...
  \caption
    ...
    (2304, 1152) IEEE 802.16e WiMAX code. BP-HL scheduling 50 iterations and NMS updates rules
    ($\alpha = 0.875$). 16-bit fixed-point.}
...
Fig.~\ref{plot:eval_ldpc_WiMAX_thr_lat} presents the obtained throughputs and
latencies for the WiMAX $\mathcal{H}$ parity matrix. On the Platinum 8168
target, the throughput is almost linear with the number of cores and the wider
instruction sets are preferable to the others. In other word, the best
throughput performance is obtained in AVX-512 and on 24 cores. If we look at the
latencies, we remark that in AVX-512 there is a marginal increase starting from
17 cores. On the EPYC 7452 CPU the performance increase is mostly linear from 1
to 15 cores. After that, the latency is increasing and the throughput gains are
reduced. Like for the Platinum 8168 target, it is preferable to use the wider
possible SIMD instructions on the EPYC 7452 CPU.
...
\begin{figure}[htp]
  ...
  \caption
    ...
    {LDPC decoder throughput and latency depending on the number of cores.
    (16200, 14400) DVB-S2 code. BP-HL scheduling 50 iterations and NMS updates rules
    ($\alpha = 0.875$). 16-bit fixed-point.}
  \label{plot:eval_ldpc_DVB-S2_thr_lat}
\end{figure}

Fig.~\ref{plot:eval_ldpc_DVB-S2_thr_lat} presents the obtained throughputs and
latencies for the DVB-S2 $\mathcal{H}$ parity matrix. This matrix is
significantly larger than the previous one. As a consequence the memory
footprint of the decoder is also higher. This highlight some limitations of the
...
instructions. On the proposed 16-bit implementation, in SSE 8 frames are
buffered, in AVX 16 frames are buffered and in AVX-512 32 frames are buffered.
...
big enough to contain all the data anymore and the number of slow transactions
...
8168 target) but later (20 cores). On the EPYC 7452 the throughout performance
results always take advantage of the increasing number of cores but in AVX,
after 16 cores the performance gains is smaller. This is due to the fact that
the data cannot be fully contained in the L2 caches anymore.
...
$128 / 32 = 4$ MB per core. This is approximately three times mores L3 memory
...
Table~\ref{tab:eval_ldpc_comparison} shows the fastest software LDPC BP
...
metric considers 50 iterations on a single core. It allows to directly compare
...
representation while our implementation run on 16-bit. It is unclear if the
8-bit decoders from the literature are able to maintain the same BER/FER
decoding performance (or approaching) as with the 32-bit floating-point
representation.
...
(quasi-cyclic). It is then possible to use the SIMD instructions inside the
decoding of a single frame. It leads to much lower latencies and we can see that
...
proposed implementation is its flexibility, it can be run on 32-bit
floating-point and 16-bit fixed-point. There is also a unique source code
...
performance can be more interesting on one or the other of the SIMD engine.
...
the compression technique for the generated SC decoders
(see Section~\ref{sec:opt_polar_sc_compression}) is evaluated. Finally, the
...
% The \bigARM and the \little clusters of cores on the \odr board are on/off in a
...
summarizes the ARM\R platforms. There is two platforms with Cortex-A15 cores and
...
the frequency boost is enabled on the Intel\R CPUs. As a convention, the
intra-frame SIMD version of the SC decoder is represented in blue in the figures
while the inter-frame SIMD version is represented in red.
...
architectures. For a software decoder such as AFF3CT, many parameters can
...
where $P$ is the average power (Watts), $\mathcal{L}$ is the latency, $K$ the
number of information bits and $F$ is the number of frames decoded in parallel
(in the inter-frame implementation $F > 1$).
...
energy than the A7-O and the A57 consumes less energy than the A15-O,
respectively. This can be explained by architectural improvements brought by the
more recent ARM64 platform. Despite the fact that the ARM64 is a development
...
number of cache misses causing RAM transactions.
% In general the code generation is effective on the intra-frame strategy whereas
% it is negligible on the inter-frame version of the code.
...
from 0.946 V to 1.170 V. The A7-O \little cluster shows that the energy consumed
by the system RAM is significant: At 250 MHz it accounts for half of the energy
cost. Indeed, at low frequency, the long execution time due to the low
throughput causes a high dynamic RAM refreshing bill. It is therefore more
interesting to use frequencies higher than 250 MHz. For this problem size and
configuration, and from an energy-only point of view, the best choice is to run
the decoder at 350 MHz. On the A15-O \bigARM cluster, the energy cost is mainly
...
\begin{figure}[htp]
  ...
  \caption
    ...
    Running on A7-O, A53 and A57 @ 450MHz.}
...
\begin{figure}[htp]
  ...
  \caption
    ...
    {Ranking of the different approaches along
     ...
     and the best version for one axis is placed further from the center.}
...
are highlighted in bold font (a CPU with L1I = 32 KB is supposed, this is
consistent with most of the today CPUs). Sub-tree folding was enabled starting
from $N=2^{12}$ because there is an overhead (at run-time) when using this
technique. \AFFECT decoder code sizes without compression are shown in
parentheses: we can observe a huge improvement, until $N=2^{14}$ the code size
never exceeds the L1I cache anymore. In~\cite{Giard2016b}, authors report that
they can't compile codes longer than $N = 2^{15}$. The proposed compression
technique allows to exceed that limit. For instance, in
Fig.~\ref{plot:eval_polar_sc_energy_implems_vs} we were able to generate
$N = 2^{20}$ decoders.
...
\begin{table}[htp]
  ...
  \caption
    ...
    codes of rate 0.84 and rate 0.9, using intra-frame SIMD. The two cross marks
    show state-of-the art performance results reported in~\cite{Sarkis2014}, for
    comparison. The AVX SIMD instructions are used.}
...
\begin{figure}[htp]
  ...
  \caption
    ...
    decoding stages (inter-frame vectorization). Squares show AFF3CT generated
    SC decoders results. Circles show the ``handwritten'' implementation results
...
previous work generated decoder results (between 10\% and 25\% higher). This is
confirmed in Table~\ref{tab:eval_polar_sc_gen_thr_comparison} which compares
\AFFECT with the state-of-the-art result samples for some specific rates
reported in~\cite{Sarkis2014}. The throughput of the inter-frame implementation
is shown in Figure~\ref{plot:eval_polar_sc_gen_thr_inter} for different
architectures. Again, the results confirm that our generic approach overtakes
handwritten code (also between 10\% and 25\% higher on x86). It worth mentioning
that the decoder from \cite{LeGal2015a} is not generated and can dynamically
adapt to various frame sizes and rates where our generated decoders cannot. To
the best of our knowledge, there is no other generated implementation with the
inter-frame SIMD strategy in the literature. By design, generated decoders are
faster than the dynamic decoder (up to 20\% on x86-based CPUs, it is less clear
on ARM\R-based CPUs).
...
decoders. Despite the additional levels of genericity and flexibility, the
proposed implementation is very competitive with its counterparts.
...
improvements detailed in Section~\ref{sec:opt_polar_scl}. Furthermore, the
...
literature~\cite{Huang2011,Zhang2012,Wu2013} while the two high-end processors
(E5-2650 and E5-2680 v3) are used for multi-threading benchmarking. Indeed,
E5-2650 and E5-2680 v3 are potentially good candidates for Cloud-RAN servers for
instance. Moreover, the code has been compiled with the GNU compiler (version
4.8) and with the \verb|-Ofast -funroll-loops -msse4.1/-mavx2| flags.

\subsection{Throughput Performance on Multi-core CPUs}

\begin{figure}[htp]
  \centering
  \includegraphics[width=1.00\textwidth]{\curChapter/fig/turbo/thr/thr}
  \caption
...
AVX instructions use vectors $2\times$ wider than those used by SSE instructions
and the inter-frame strategy loads twice the number of frames to fill these
vectors. Thus, for $K > 4096$, in AVX, the memory footprint exceeds the L3 cache
optimal occupancy and the performance is driven by the RAM bandwidth. Then, as
$K$ increases the number of RAM accesses increases and there is not enough
...
\begin{figure}[htp]
  ...
  \caption
    ...
    measurements were conducted on i7-4960HQ with the
...
on the i7-4960HQ CPU target. For small codewords ($K=1024$) it is more energy
efficient to resort to AVX. But this is not so clear on larger codewords
($K=6144$) since with 3/4 cores, the code using SSE outperforms the AVX one.
...
6 iterations on a single core. It allows to directly compare the throughput of
the listed decoders. The FER decoding performance of the proposed decoders are
shown in Fig.~\ref{plot:aff3ct_turbo_quantification}. One can note that using
the 8-bit decoder leads to BER/FER degradations.

The variety of CPU targets and algorithmic parameters allows to
...
price of an increased latency (from 2212 $\mu$s to 2837 $\mu$s) and memory
footprint. The implementation from~\cite{LeGal2019a} outperforms the throughput
(up to 2,5 times higher) and the latency performance of our proposed decoder on
the E5-2680 v3 target. The other works~\cite{Zhang2012,Wu2013,LeGal2019a} use
the intra-frame SIMD strategy. It results in significantly reduced latencies
compared to the proposed implementation. The intra-frame implementations use
less memory than the inter-frame strategy and it is possible to take advantage
of wider SIMD instructions for the selected code. It mainly explains the
throughput performance difference between the proposed implementation and the
implementation from~\cite{LeGal2019a}. Also, the decoder has been specialized
for the LTE interleaver. However, in the LTE standard the EML-MAP turbo decoder
comes with a limited intrinsic parallelism of 8 (corresponding to the 8 trellis
states). To fill the SIMD registers, the intra-frame implementations require to
modify the algorithm to expose more parallelism. This modification leads to
non-negligible decoding performance losses. At 0.7 dB, none of the
state-of-the-art implementations are able to match the reference decoding
performance of the EML-MAP algorithm except our proposed implementation in
16-bit. Even considering our 8-bit implementation, none of the other works are
reaching the same level of decoding performance.
...
and the number of cores used, the throughput and latency performance can be more
interesting on one or the other of the SIMD engine. Moreover, the proposed
...
% state-of-the-art is provided for the mains channel codes.
...
information throughput is presented ($\mathcal{T}_i$ considering $K$ bits).
...
\emph{normalized throughput} ($\mathcal{NT}$) is different for each HoF but the
idea is to normalize the throughput with a representative number of
iterations/lists ($I$). Here is the normalized throughput:
\begin{equation}
...
power consumption, throughput and latency are all important aspects that are
discussed.
...
compiler reaches 3.51 Mbps utilizing all four physical cores of the processor
(SMT on). Log-MPA improves the performance to 6.37 Mbps benefiting from
elimination of the exponential calculations, still in \verb|-O3|. However, using
...
implementations, while the energy consumption is reduced. Utilizing AVX
increases the average power consumption of MPA and log-MPA from 35 to 40 Watts
but throughput and latency are improved by much larger factors. It means that
the overall energy consumption have been decreased with AVX.
...
decoding task. Despite these benefits, the Xeon Phi\TM Knight Corners suffers
from two main disadvantages: 1) the KNC instructions diversity is reduced
...
compared to the ARM\R-based implementations. The MPA decoding algorithm exhibits
its best performance on this platform when cross compiled using \verb|-O2 -mmic|
flags by an Intel\R ICPC compiler. Using fast math options such as
\verb|-no-prec-div| \verb|-no-prec-sqrt| \verb|-fp-speculation=fast|
\verb|-fp-model-fast=2| do not change the results significantly with the Intel\R
compiler.
...
of SCMA decoders on Cloud-RAN servers as it allows significant energy savings.
...
Phi\TM obtains the best throughput while the Cortex-A57 has the lowest energy
consumption. In the case where the number of users in the cloud is increased,
the results presented in this section are scalable up to the number of
processing units dedicated to them.
...
codewords are generated. This technique allows to bench only the AWGN channel
...
Section~\ref{sec:opt_simu_awgn} is used. The SNR is set to $E_b/N_0 = 4.5$~dB.
The BPSK demodulator implementation is also vectorized and its implementation
has not been detailed in the manuscript as it is trivial and can be resumed to
the multiplication of the AWGN channel output by a constant factor ($\bm{l} =
\bm{y} \times \frac{2}{\sigma^2}$). Then, the demodulator 32-bit floating-point
output data is converted in a 8-bit fixed-point representation thanks to the
quantizer task (see Section~\ref{sec:opt_simu_quantizer}). The decoder is the
8-bit polar FA-SSCL presented in Section~\ref{sec:ctx_polar_ascl} an in
...
After the decoding process, the 32 CRC bits are extracted from the polar
codeword and $K_2 = 1723$ bits are returned. This operation consists in copying
the $K_2$ first bits of the $K_1$ decoder output bits. Finally, the monitor
checks if all the bits are equal to zero, if not, errors are counted. All the
tasks of the proposed communication chain are fully vectorized with the \MIPP
wrapper and the intra-frame SIMD strategy is used.
...
proposed simulator. \AFFECT can adapt to various CPU architectures.
...
CPU comes from the best performance of the Gold 6140 CPU when simulating the
AWGN channel. Indeed, the Gold 6140 CPU takes advantage of its doubled SIMD
length (AVX-512) compared to the AMD\R EPYC CPU (AVX2). The ARM\R ThunderX2
CPU comes with the worst single threaded general performance. The Intel\R Xeon
Phi\R is also very close to the ARM\R CPU performance. This is not surprising as
these processors have not been designed for the single core performance but for
...
\begin{figure}[htp]
  ...
  \caption
    [\AFFECT simulation of a (2048,1723) Polar code, FA-SSCL decoder $L=32$.]
    {\AFFECT simulation of a (2048,1723) Polar code, FA-SSCL decoder $L=32$,
    BPSK modulation, AWGN channel, $E_b/N_0$ = 4.5~dB (BER = 4.34e-10, FER =
    5.17e-08).}
...
run assigns at most one \AFFECT thread to each hardware thread, thus, since the
...
% In Figure~\ref{plot:simu_speedup_throughput}, an $N=2048$ and $K=1723$
...
% last SNR point of the blue curve in Figure~\ref{fig:ctx_bfer_polar}). The
...
% \Cxy{11} threads. The intra-frame vectorization is applied (c.f.
% Section~\ref{sec:opt_vec_intra}) to take advantage of the algorithms intrinsic
% level of parallelism, when the multi-threaded parallelism is used,
To reduce the simulation time by multiplying the number of concurrent
communication chains, thanks to the independence property of Monte Carlo
simulations.
% In order to achieve highest possible throughputs, the receiver part of the simulator is
...
% 64/32-bit floating-point and 16-bit fixed-point.
...
performance. The ARM\R target and the Xeon Phi\TM are not competitive in term of
throughput even when all the hardware threads are used. It is possible that
these targets are more adapted for SDR uses (like for the Cloud-RAN) when power
consumption matter more. However, for simulation purpose, it is clear that the
AMD\R EPYC CPUs are more interesting. Those results also demonstrate the
...
speedup are almost linear with the number of nodes: This is expected because
there are very few communications between the various MPI processes. Note that
the super-linear scaling is due to the measurements imprecision.
...
In this chapter we evaluated the implementations of the LDPC, polar and turbo
decoders proposed in this thesis. The throughput, the latency and the energy
efficiency have been studied and compared with other works. For the LDPC and the
turbo decoders, the inter-frame strategy has been used and leads to throughputs
comparable with the state-of-the-art performance. However, the latencies are not
competitive with the intra-frame implementations we found in the literature, as
expected. These implementations are then more oriented for simulation purpose or
for real-time applications that do not require low latency like the video
streaming for instance. For the polar decoders, both the inter-frame and
intra-frame strategies has been implemented. It results in a complete framework
that can adapt to a lot of applicative contexts. The proposed decoders are
ones of the fastest in the literature. There are also able to be very flexible
with the dynamic implementations or specialized for highest possible performance
with the source code generation technique. For all the proposed decoders (LDPC,
polar and turbo), the level of genericity is one of our main contribution. The
implementations are able to adapt to various CPUs architectures as well as to
support many algorithmic variants. Moreover, each of the presented
implementations are able to work close to the reference decoding performance.

The software decoder Hall of Fames (HoFs) are introduced. These HoFs are
exhaustive surveys of the software decoders we found in the literature. The
proposed decoders are reported as well as the other state-of-the-art works.
These HoFs allow to compare CPU and GPU implementations. Some metrics like the
...
than the GPU works in terms of throughput, latency and energy. One of the main
issue of the GPU-based implementations is the required transfer time between the
CPU and the GPU. An other main issue comes from the intrinsic architecture of
the GPU that requires a lot of parallelism to be efficient. This is not always
possible to take advantage of this high level of parallelism in the channel
decoding algorithms. As a consequence, in general, the CPUs are more adapted to
low latency implementations than the GPUs.
...
error-rate level. To the best of our knowledge, \AFFECT is one of the fastest
FEC simulator.
