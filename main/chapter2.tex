...
functions. Figure~\ref{lst:polar_f_g_h_simd} details the SIMD implementation of
...
Figure~\ref{lst:polar_f_g_h_simd}, each operation is made on multiple elements
...
possible memory layout is the one given in Figure~\ref{fig:sc_decoder}. In this
layout, the partial sums $\hat{s}$ of each node is stored in a dedicated array.
Therefore, a memory of size $2N-1$ bits is necessary in the SC decoder, or
$L(2N -1)$ bits in the SCL decoder. This memory layout is described
