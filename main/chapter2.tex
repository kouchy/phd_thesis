...
\begin{figure}[htp]
  \centering
  \includegraphics[width=1.00\textwidth]{polar/tree_pruning_example}
  \caption{Example of tree pruning on a small binary tree ($N = 8$). The tree is
    cut and the computations are versioned according to the location of the
    frozen bit}
...
\verb|REP| nodes degrade the decoding performance in a 8-bit representation
because of accumulation (red triangles curve). Indeed, it is necessary to add
all the LLRs of a \verb|REP| node together in order to process it, which may
...
\begin{itemize}
  \item \xmark~décodeur Turbo LTE (3G/4G)
  \item \xmark~treillis
\end{itemize}
...
\begin{itemize}
  \item \xmark~décodeurs génériques (BP-flooding/HL/VL) sur les "update nodes"
\end{itemize}
