...
key of efficient software implementations. A specific wrapper as well as a
sub-set of generic parallelism levels are proposed.

The following sections are dedicated to the efficient software implementation of
...
of expressiveness and of performance for the algorithms dedicated to channel
codes. Our contributions are:
\begin{itemize}
...
\end{itemize}
\MIPP programming model is not too far from intrinsics, enabling a good control
...
\MIPP is also Open-source (under the MIT license) and the full code is available
on GitHub\footnote{\MIPP repository: \url{https://github.com/aff3ct/MIPP}}.
...
following instruction \verb|"m ? a+b : src"| performs an addition and a masking
operation:
...
{\noindent
Likewise, a scalar assigned to a vector set all elements to this value.
}
...
{\noindent
A shortened version this previous function is also defined as:
...
advantage of conventional instruction sets. The existing alternatives can be
...
to the library approach since the main objectives are to to maximize
performance, maximize portability and deal with existing \Cxx codes. In order to
let the compiler inline library calls, which is critical for the intended SIMD
programming model purpose, such library are usually header-only. Thus, we refer
to them as \textit{wrappers} instead of \textit{libraries}.
...
hand-coded for each specific instruction set. The intrinsics version considers
the ``\emph{golden}'' model.
...
lacks support for NEON and AVX-512 (which are currently under progress).
...
It performs well in 32-bit NEON, SSE and AVX. But, it lacks from AVX-512.
Support of the 64-bit types is not planed since it is not useful in traditional
image computations.
...
medium level interface. Moreover, the obtained speedups are close to the
intrinsics version.
...
The vectorization is a key feature to develop high performance implementations
of signal processing algorithms. One of the main constraint of these algorithms
is their low latency requirement. A typical signal processing latency ranges
between one microsecond to a few nanoseconds. Therefore, the famous
multi-threading parallelism is not well-suited to speedup a single signal
processing algorithm. Indeed, the thread creation time and the extra cost of the
threads synchronization are too high compared to the expected latency of these
...
AVX-512 ISA, there is 512-bit in the registers. If the computations are made on
...
current CPUs. The next subsections detail the vectorization strategies that we
have identified to effectively implement signal processing algorithms on CPUs.
...
permutation extra-instructions have to be added. These extra-instructions are
...
then $F = p_\text{SIMD}$ frames are used to fulfill the SIMD registers. The
...
solution has achieved poorer performance on current CPU architectures. The
selected version with the reordering operations comes with a much better data
locality at the cost of extra operations before and after the SIMD computations.
...
$p_\text{SIMD} = 16$ and if the algorithm has only an intrinsic parallelism of
...
This process can also take an important amount of time and it has been
optimized. The proposed implementations are briefly evaluated with the protocol
defined in Section~\ref{sec:opt_mipp_protocol}.
...
$U_2$ are independent variables. So the algorithm intrinsic parallelism directly
...
AVX-512, compared to the sequential code (can be auto-vectorized). It also gives
...
\begin{table}[htp]
  ...
  \textbf{Throughput} (Mb/s) & 300.6         & 3541.4        & 5628.3        \\ %\hline
...
\verb|for-loop|. \MIPP enables to convert floating-point data types to integers
...
the \verb|(int8_t)| cast. Table~\ref{tab:opt_simu_quantizer_speedup} summarizes
...
AVX-512BW compatible CPU would be required and the Xeon Phi\TM 7230 is not.
...
the intra strategy has also be studied~\cite{LeGal2019a,Xu2019}, the achieved
...
proposed in the literature always target a specific scheduling and update rules.
We found this is a limitation to explore new trade-offs. Our contribution is to
propose a generic implementation with a clear separation of concerns between the
scheduling and the updates rules.
...
\verb|Update_rules| class can be all the update rules like the SPA, the MS, the
OMS, the NMS, etc. An \verb|Update_rules| class has to implement the interface
composed by the \verb|begin_chk_node_in|, \verb|compute_chk_node_in|,
...
the \verb|Update_rules_MS| class is vectorized thanks to \MIPP. It results a
...
performance of a new algorithm and to combine it with all the previously
...
could be interesting to automate this process.
...
\begin{itemize}
  \item The tree traversal is sequential. But $f$, $g$ and $h$ (cf.
    ...
    node, these node computations can be parallelized or vectorized (cf. the
  ...
  \item Similarly, multiple frames can be decoded concurrently, with parallel or
    ...
    latency, which is also one important metric of the application
    (cf.~\cite{LeGal2015a}).
\end{itemize}
...
rewriting rules listed in Fig.~\ref{fig:opt_polar_patterns} using by a depth
...
rules of the third line describe partial cuts and specialization. For instance,
...
first. Rule selectors can match on node tags and/or node levels (leaf, specific
...
flexible, extensible proposed architecture. Indeed, the rewriting rule set can
also be enriched with rules for specific ranges of code rate. For instance, the
...
of the fact that higher rate codes involve higher information throughputs.
...
all the elements of the \verb|neg_la| and \verb|lb| registers is executed in
only one CPU cycle. We also tried an auto-vectorized approach but even if all
the routines were well vectorized (from the compiler report), the performance
was, at least, 3 times slower than the MIPP handwritten versions. The template
\verb|N_ELMTS| parameter is not used in the proposed \verb|API_polar_SIMD|
implementation. The interest of this parameter will be explained in the next
section.
...
level 2 and above. Thus, intra-frame strategy is only effective on trees that
can be heavily pruned from these numerous scalar nodes. Even if the tree is
heavily pruned some nodes can't be fully vectorized in the lower layers. In this
context, the building blocks of the polar API enable to automatically switch
from SIMD to sequential implementations.
...
the best of our knowledge) to support both multi-precisions (32-bit, 16-bit and
...
/ N$), position of the frozen bits. All these parameters are  statically known
at compile time. Thus, the recursive tree traversal code structure and the
...
the decision. This method gives similar decoding performance than performing the
cyclic redundancy check of each candidate and then to select the one with the
...
%     elle doit aller dans la Section~\ref{sec:opt_vec_inter} à terme
...
sequential, bounding the speedup beyond the capabilities of SIMD units.
Inter-frame parallelism has been proposed in~\cite{Wu2010,Wu2011,Zhang2012,
Wu2013}. Multiple codewords are decoded in parallel, it improves the memory
...
has to be performed at the end of the turbo decoding. These reordering
...
%The presented decoder implementation is available in the \AFFECT software.
...
instructions (SSE, AVX and NEON). It provides possible trade-offs between
...
In this section, we presented an high throughput implementation of the turbo
...
128-bit, 256-bit or 512-bit. It reduces the number of iterations in the
...
% Vectorized
% instructions such as AVX are accessible in \Cxx through intrinsic
...
with an Array of Structures (AoS) for complex numbers. In this data layer, the
...
instructions. It depends on the data type \verb|T| and the SIMD ISA. The
...
$-1/2\sigma^2$. Finally the exponential function is performed according to
\eqref{eq:ctx_scma_7}.
...
well-known padding technique has been applied. By this way, the read and written
...
are detailed and the corresponding \MIPP implementations are given. Other
well-known optimization techniques, such as loops unrolling, avoiding type
...
efficiency on current CPUs. Thus, a main contribution in this chapter is the
proposition of \MIPP: a wrapper for the SIMD instructions. The idea is to
abstract data types and SIMD ISAs in order to propose ``universal'' and
efficient implementations of digital communication receiver algorithms. We show
that \MIPP is almost as efficient as the specific intrinsic functions (or
assembly code) and it is able to operate on the floating-point representation as
well as fixed-point ones. For digital communication receiver algorithms,
fixed-point representations are very interesting as they result in an increased
SIMD parallelism.
...
The four last sections focus on the efficient software implementations of the
algorithms presented in Chapter~\ref{chap:ctx} (LDPC, polar, turbo and SCMA
decoders). The LDPC BP decoders, the polar SC decoders and the turbo decoder are
compatible with the inter-frame SIMD strategy while the polar SC/SCL decoders
and the SCMA decoders are compatible with the intra-frame SIMD strategy.
Depending on the code families, we focus on different constraints. The LDPC BP
decoders have been implemented to support many variants and thus to maximize the
flexibility at the cost of lower throughputs and higher latencies compared to
other works. In the polar decoders, flexibility as well as very aggressive
optimizations are considered, combined and opposed. The turbo decoder focuses on
achieving the highest possible throughputs and some specializations are made for
the LTE standard. Finally the SCMA decoder implementation tries to propose a
compromise between high throughputs and low latencies.
