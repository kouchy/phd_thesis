...
\begin{equation}
\label{eq:alpha}
 \alpha_j^{k+1} = max^*_{i \epsilon F} \{ \alpha_i^k + \gamma(s_i^k, s_j^{k+1}) \}
\end{equation}
...
trellis traversal in Eq.~\ref{eq:alpha}. The vector of backward state metrics,
denoted by $\mathbf{\beta^k} = [\beta_0^k, ..., \beta_7^k]$, is computed as:
\begin{equation}
\label{eq:beta}
 \beta_j^k = max^*_{i \epsilon B} \{ \beta_i^{k+1} + \gamma(s_j^k, s_i^{k+1}) \}.
\end{equation}
...
effective speedup, as shown in Section~\ref{sec:results}.
...
\begin{table}
  ...
  % {\small\resizebox{\linewidth}{!}{
  \begin{tabular}{c | c | c | c}
    \textbf{CPU}          & \textbf{P1} : Xeon E5-2650 & \textbf{P2}: Core i7-4960HQ & \textbf{P3}: Xeon E5-2680v3 \\
    \hline
    \textbf{Intel Arch.}  & \textit{Ivy Bridge} Q1'12  & \textit{Haswell} Q4'13      & \textit{Haswell} Q3'14      \\
    \hline
...
the \emph{Intel Turbo Boost} (ITB) technology was disabled on their CPUs. For
