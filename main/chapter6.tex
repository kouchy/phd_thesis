...
to the Software Defined Radio (SDR). The first sections discuss of the existing
models and solutions and motivate the need of a new dedicated language for the
SDR.

In a second part a description of the developed language is given and
separated in two types of blocks: 1) the serial blocks and 2) the parallel
blocks. Follows some implementations details about the realization of these
blocks.

The third part is a concrete use case of the proposed eDSL with the
realization of a complete digital communication system in software. The DVB-S2
standard is presented from an applicative point of view (transmitter and
receiver) and evaluated on a specific CPU target.
...
implementation of new algorithms. Still, we show this is a limitation to meet
high throughputs and low latencies constraints on current GPPs architectures.
...
synchronous dataflow model specialized to the relevant characteristics of FEC
communication chains enabling to perform more aggressive optimizations than
...
unlike a dataflow filter, can have an internal memory to store temporary data
for instance. If the life time of the private data exceeds the task execution
then the data will be contained by the \emph{module}. Additionally a set of
tasks can share the same internal/private memory, in that particularly case,
multiple tasks are regrouped in a single module. This behavior is not
...
task execution which is a very expensive operation.
...
The proposed eDSL is targeting streaming application and more precisely signal
...
needs to specify the first task of the chain ($t_1$) and the sequence will
automatically follow the binding until the last task ($t_4$).
...
will restart from $t_3$. The index $i$ of the $t_i$ task represent the execution
order. The $t_5$ and $t_6$ last tasks have to be given by the user because there
are other tasks bound to their output sockets: the analysis cannot guess the
end of the sequence alone.
...
issue the well-known pipelining strategy can be used to be able to increase the
sequence throughput up to the slowest task throughput. The proposed eDSL comes
with a specific \emph{pipeline} component to this purpose. The pipeline takes
...
will be discussed in the next section. It worth mentioning that is is not
possible to split the sub-sequence 2 of a loop in separate pipeline stages.

\section{Implementation Details}

\subsection{Implicit Rules}

In the proposed DSL, an input socket can only be bound to one output socket
while an output socket can be bound to multiple input sockets. A task can only
be executed if all its input sockets are bound. The scheduling of the tasks in a
sequence is defined by the binding order. The general rule in the sequence
...
these tasks in a predefined order. The user cans also execute tasks explicitly,
outside sequences. When using a sequence, the analysis process is able to match
...
been introduced: in a sequence each task is encapsulated a \emph{process} (this
...
\begin{figure}[htp]
  ...
  \\
  \subfloat[][Automatic parallelization of a pipeline description: sequence duplications, $1$ to $n$ and $n$ to $1$ adaptors creation and binding.]{\includegraphics[width=1\textwidth]{dsl/pipeline/pipeline_adp}\label{fig:dsl_pipeline_adp}}
...
of $t_5$ and $t_6$ tasks. We assume that the cumulate execution time of $t_2$,
$t_3$ and $t_4$ is approximatively four times higher than $t_1$. This knowledge
motivates the cutting of the sequences 1, 2 and 3. There is no need to split the
$t_5$ and $t_6$ tasks in two sequences because the overall throughput is limited
by the slowest stage ($t_1$ here). The sequence 2 is duplicated four times to
increase its throughput by four as we know that its latency is approximatively
four times the sequence 1.
...
execution this not possible anymore because a many threads are running
...
\emph{tail} counter by one. If the buffer is full the producer (\emph{push 1 and
n} tasks) has to wait, same for the consumer (\emph{pull 1 and n} tasks) if the
buffer is empty. We implemented both active and passing waiting.
...
Fig.~\ref{fig:dsl_pipeline_adp} is an example of a simple chain of tasks. More
...
An other pathologic case is when a task $t_1$ is in stage 1 and its output
socket is bound to an other task $t_{x}$ which is in the stage 4. To work, the
pipeline adaptors between the stage 1 and 2 and 2 and 3 automatically
synchronize the data of the $t_1$ output socket.
...
\begin{figure}[htp]
  \centering
  \subfloat[][Waiting phase and learning phase 1 \& 2.]{\includegraphics[width=1.0\linewidth]{dvbs2/receiver/receiver_learning}\label{fig:dvbs2_receiver_learning}}
  \\
  \subfloat[][Learning phase 3 \& transmission phase.]{\includegraphics[width=1.0\linewidth]{dvbs2/receiver/receiver_transmission}\label{fig:dvbs2_receiver_transmission}}
  \caption
    [DVB-S2 receiver.]
    {DVB-S2 receiver.}
...
the tasks have to be re-bound for the \emph{learning phase 3}
...
inserted by the \emph{Framer PLH} task in the transmitter to recover the first
and last symbols. One can notice that the \emph{Synchronizer Timing} module is
composed by two separated tasks (\emph{synchronize} or $t^\text{Rx}_5$ and
\emph{extract} or $t^\text{Rx}_6$). This behavior is different from the other
\emph{Synchronizer} modules. The \emph{synchronize} task ($t^\text{Rx}_5$ or
$t^\text{Rx}_{3,4,5}$) has two output sockets, one for the regular data and
...
scheduling (10 iterations) and with the min-sum update rules. One can notice
that each DVB-S2 configuration has a well-separated SNR predilection zone.
...
GHz and the \emph{Turbo Boost} mode has been disable for the reproducibility of
...
throughputs with each others. This is why we introduced the normalized average
throughput ($\mathcal{N}_\text{Avg}$) which is the average throughput
...
($t^\text{Rx}_{17}$) that take 59\% of the time. $t^\text{Rx}_{17}$ should not
...
adapt the cores charge and energy can be saved. In the
...
sequential while it uses around 30 GB in parallel (28 threads), this is linear
with the number of threads. The memory usage increases because of the sequence
duplications in the stage 7. The duplication operation takes about 20 seconds
and is made at the very beginning of the program. It is worth mentioning, that
the amount of memory was not a critical resource so we did not try to reduce it
overall occupancy.
...
time spent in the de-interleaving task ($t^\text{Rx}_{15}$) is higher than the
time saved in the \emph{Synchronizer Timing} task ($t^\text{Rx}_{5}$).
