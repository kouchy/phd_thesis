...
\begin{frame}{Context}
  ...
  \begin{figure}[htp]
  \centering
  \includegraphics[scale=0.40]{pic/com_num}
  \end{figure}
  \vfill
\end{frame}
...
\begin{frame}{Software-defined Radio}
  ...
  \begin{column}{.3\textwidth}
    ...
    \end{tikzpicture}
    \caption*{\footnotesize{5G and future}}
    \end{figure}
    }
  \end{column}
  \end{columns}
...
\begin{frame}{Digital Communication Systems}
  ...
  \begin{itemize}
    ...
    \item<2-> Source coding: seek maximum conciseness in the expression of a message
...
\begin{frame}{Functional Simulation}
  ...
  \begin{tikzpicture}[scale=0.65, every node/.style={transform shape}]
    ...
    \node[text=Paired-5, align=center] at (0.3,2) {output: FER};
    }
  \end{tikzpicture}
  \end{figure}
  ...
  \begin{itemize}
    ...
    \item<3-> Monitor: compares initial message $\bm{u}$ with decoded message $\bm{\hat{u}}$
    \item<4-> All the processing represent a \textbf{simulation chain}
  \end{itemize}
  }
  \only<5->{
  \begin{itemize}
    \item<5-> \textcolor{Paired-3}{$C$}, \textcolor{Paired-3}{$K$}, \textcolor{Paired-3}{$N$}: characteristics of channel code
    \item<6-> \textcolor{Paired-1}{$m$}, \textcolor{Paired-1}{$M$}, \textcolor{Paired-1}{$s$}: configurations of channel model
    \item<7-> \textcolor{Paired-5}{Frame Error Rate (FER)} performance: estimated by the monitor
  \end{itemize}
  }
\end{frame}
