...
\begin{frame}{Coding Scheme}
  ...
  \begin{itemize}
    \item<1-> $\bm{u}$: initial message ($K$ bits)
    \item<1-> $\bm{c}$: codeword ($N$ bits, $K < N$)
...
\begin{frame}{SC Intrinsic Parallelism}
  ...
  \vspace{1.cm}
  \begin{itemize}
    \item Parallelism decreases with tree depth (intra-frame parallelism)
    \vspace{0.3cm}
    \item<5-> Regular memory access pattern
    \vspace{0.3cm}
    \item<6-> \textbf{Problem:} limited SIMD efficiency near leaves
    \vspace{0.3cm}
    \item<7-> \textbf{Solution 1:} reduce number of nodes that have a limited amount of parallelism
  \end{itemize}
  \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Simplified Successive Cancellation (SSC) Algorithm}
  ...
  \begin{itemize}
    \item SC simplifications proposed in literature~\cite{Alamdar-Yazdi2011,Sarkis2014a}
    \begin{itemize}
      \item Introducing new type of nodes {\color{bleuUni}\Large\MVRightarrow} \textbf{Tree pruning}
      \item<8-> \textbf{2047 nodes pruned to 291 nodes} ($N = 1024$, $R = 1/2$)
    \end{itemize}
  \end{itemize}
  \vspace{0.5cm}
  ...
  \begin{columns}[t]
    \begin{column}[T]{5.0cm}
      ...
      \begin{tikzpicture}
        ...
        \node[lb] (r0_e7) at (\lft+4.9, -\vs) {\footnotesize{$\lambda_{b}^3$}};
    ...
    \begin{column}[T]{5.0cm}
      ...
      \begin{tikzpicture}
        ...
        \node[lbh] (r0_e7) at (\lft+4.9, -\vs) {\footnotesize{$\lambda_{b}^3$}};
...
\begin{frame}{Sub-tree Folding Algorithm}
  ...
  \vfill
  \begin{itemize}
    \item Find similar \textbf{sub-patterns} and \textbf{create functions} for them
    \item \textbf{Compromise} between the \textbf{number of functions calls} and the \textbf{binary size}
    \item \textbf{Threshold} to avoid functions that contain too \textbf{few computations}
  \end{itemize}
\end{frame}
...
\begin{frame}{Impact of the Compression Techniques on the Decoder Binary Size}
  ...
  \vfill
  \begin{itemize}
    \item \textbf{Problem:} Codewords$~> N = 2^8$ exceed CPU instruction cache
    \vspace{0.2cm}
    \item<2-> \textbf{Solution 1:} Do not store intermediate memory address offsets
    \vspace{0.2cm}
    \item<3-> \textbf{Solution 2:} Implement a sub-tree folding algorithm
  \end{itemize}
\end{frame}
  ...
  \begin{column}[T]{6cm}
    ...
    \begin{tikzpicture}
      \begin{axis}[/pgfplots/table/ignore chars={ },
                   ...
                   xmin=4, xmax=65536, grid style={dashed, gray!30},
                   legend pos=south east, legend columns=1]
