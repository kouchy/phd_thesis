...
\begin{frame}{Tasks and Sequence}
  ...
  \begin{itemize}
    ...
    \item \textbf{Oriented graphs} have to be supported \textbf{to map real SDR systems}
    \item Tasks scheduling is resolved at the construction of the sequence
    % \begin{itemize}
...
\begin{frame}{Parallelization Techniques: Sequence Duplication}
  \begin{figure}[!h]
    \centering
    \scalebox{.65}{
    \begin{tikzpicture}%[scale=\tikzscale]
      ...
      \node[sseq, label={[Paired-9]below:Sub-sequence $t$}, fit=(tttt1) (tttt4)] {};
...
\begin{frame}{Parallelization Techniques: Pipeline}
  \vfill
  \begin{figure}[!h]
    \centering
    \scalebox{.65}{
    \begin{tikzpicture}%[scale=\tikzscale]
      ...
      \node[seq, label={[Paired-11]below:Sequence 2}, fit=(t2_si) (t2) (t3) (pppt4) (pppt4_so)] (seq2) {};
      \node[seq, label={[Paired-11]below:Sequence 3}, fit=(t5_si) (t5) (t6)                   ] (seq3) {};

      \node[pip, minimum height=3.0cm, minimum width=12.0cm, label={[Dark2-8]below:Pipeline}, fit=(seq1) (seq2) (seq3)] {};
    \end{tikzpicture}
    }
  \end{figure}
  \vfill
  \begin{itemize}
    ...
    \item A pipeline is built from a list of sequences
    \item The sequence duplication technique is still possible in parallel stages
  \end{itemize}
  \vfill
\end{frame}
...
\begin{frame}{Setup \& Objectives}
  ...
  \begin{itemize}
    ...
    \item<5-> \textbf{Industrial constraint}: 30 $\sim$ 100 Mb/s
  \end{itemize}
  \vspace{-0.1cm}
  % \vfill
  \pause
...
\begin{frame}{Receiver (Rx)}
  ...
  \begin{figure}[!h]
    ...
    \begin{tikzpicture}%[scale=\tikzscale]
      ...
      \only<3->{
      \node[seq, minimum height=3.5cm, minimum width=17.0cm,  label={[Paired-11]below:Stage 7 (parallelized with the sequence duplication technique)}, fit=(t14_si) (t18_so)                                                      ] (seq7) {};
      }
...
\begin{frame}{Evaluation}
  ...
  \begin{figure}[!h]
    ...
    \begin{axis}[/pgfplots/table/ignore chars={|}, %footnotesize,
      ...
      \addplot[mark=square,    Paired-1,    semithick, mark options={solid}] table [x=Eb/N0, y=FER] {../main/chapter5/fig/dvbs2/bfer/dat/data_QPSK_R_3_5_BB.txt};  \label{plot:llline1}
      \addplot[mark=o,         Paired-1!60, semithick, mark options={solid}, domain=0:0.2, error bars/.cd, x dir=both,x fixed=0.2, y dir=both,y explicit, error bar style={solid}] table [x=Eb/N0, y=FER] {../main/chapter5/fig/dvbs2/bfer/dat/data_QPSK_R_3_5_rad.txt}; \label{plot:llline3}
      \addplot[mark=square,    Paired-3,    semithick, mark options={solid}] table [x=Eb/N0, y=FER] {../main/chapter5/fig/dvbs2/bfer/dat/data_QPSK_R_8_9_BB.txt};  \label{plot:llline4}
      \addplot[mark=o,         Paired-3!60, semithick, mark options={solid}, domain=0:0.2, error bars/.cd, x dir=both,x fixed=0.2, y dir=both,y explicit, error bar style={solid}] table [x=Eb/N0, y=FER] {../main/chapter5/fig/dvbs2/bfer/dat/data_QPSK_R_8_9_rad.txt}; \label{plot:llline6}
      ...
      \coordinate (legend) at (axis description cs:0.85,1.05);
    \end{axis}
